...
Ltac check_hyp_is h stmt :=
 let Hf:=fresh in let HB:=fresh in
...
|  |- forall x, ?p => fail 1 "Not a fix statement"
 end.
...
Example test_letsprove_2 : forall P Q: Prop, P -> P \/ Q.
Proof.
Fix P.
Fix Q.
Assume H : P.
...
Example test_by_cases_1 : forall A B : Prop, A \/ B -> B \/ A.
Proof.
Fix A.
Fix B.
Assume H: (A \/ B).
...
Lemma test_split_1: forall A B : Prop, A -> B -> A /\ B.
Proof.
Fix A.
Fix B.
Assume HA : A.
